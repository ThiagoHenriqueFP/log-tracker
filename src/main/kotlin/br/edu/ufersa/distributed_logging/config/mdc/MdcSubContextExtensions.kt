...
fun CoroutineScope.launchWithNewInternalId(
    ...
    subContextManager.executeWithNewInternalId(block)
}

/**
 * Extensão para CoroutineScope que executa um bloco com correlation ID interno específico
 *
 * @param subContextManager Instância do MdcSubContextManager
 ...
 * @return Job da coroutine
 */
fun CoroutineScope.launchWithInternalId(
    subContextManager: MdcSubContextManager,
    internalId: String,
    block: suspend () -> Unit
): Job = this.launchWithMdc {
    subContextManager.executeWithInternalId(internalId, block)
}

/**
 * Extensão para CoroutineScope que executa um bloco com profundidade incrementada
 *
 * @param subContextManager Instância do MdcSubContextManager
 * @param block Bloco a executar com profundidade incrementada
 * @return Job da coroutine
 */
fun CoroutineScope.launchWithIncreasedDepth(
    subContextManager: MdcSubContextManager,
    block: suspend () -> Unit
): Job = this.launchWithMdc {
    subContextManager.executeWithIncreasedDepth(block)
}
...
class MdcContextScope(private val subContextManager: MdcSubContextManager) {
    ...
    suspend fun subContext(contextName: String? = null, block: suspend MdcContextScope.() -> Unit) {
        subContextManager.executeWithNewInternalId {
            if (contextName != null) {
